...
const xlsx = require('xlsx'); // Make sure you have the xlsx package installed
...
app.get('/export-excel', async (req, res) => {
  try {
    // 1. Fetch submission data from your database or data source
    const submissions = await getSubmissionsFromDatabase(); // Replace with your database query

    // 2. Convert data to a 2D array format for xlsx
    const worksheetData = [
      ['Student ID', 'Teacher', 'Submission Time', 'IP Address', 'Browser', 'OS', 'Device'], // Header row
      ...submissions.map(sub => [
        sub.student_id,
        sub.teacher,
    ...
    // 5. Send the Excel file as a download response
    res.setHeader(
      'Content-Disposition',
